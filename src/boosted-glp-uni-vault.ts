...
} from "../generated/BoostedGlpUniVault/BoostedGlpVault";
import {
  VaultPricePerShare,
  VaultTVL,
  VaultTotalSupply,
...
export function handleDeposit(event: DepositEvent): void {
  ...
  const userBalanceEvent = new UserBoostedBalanceEvent(
    `uni:deposit:${event.transaction.hash.toHex()}`
  );
  const invRatio = BigInt.fromString("1").div(pricePerShareUNI);
  ...
  userBalanceEvent.token = BOOSTED_UNI_VAULT_ADDRESS.toHexString();
  ...
  userBalanceEvent.to = BOOSTED_UNI_VAULT_ADDRESS.toHexString();
...
export function handleDeposit1(event: Deposit1Event): void {
  ...
  const userBalanceEvent = new UserBoostedBalanceEvent(
    `uni:deposit1:${event.transaction.hash.toHex()}`
  );
...
export function handleWithdraw(event: WithdrawEvent): void {
  ...
  const userBalanceEvent = new UserBoostedBalanceEvent(
    `uni:withdraw:${event.transaction.hash.toHex()}`
  );
  ...
  userBalanceEvent.assets = event.params.assets;
  userBalanceEvent.shares = event.params.shares;
  userBalanceEvent.from = BOOSTED_UNI_VAULT_ADDRESS.toHexString();
...
export function handleTransfer(event: TransferEvent): void {
  ...
  if (from != ZERO_ADDRESS) {
    const idFromTotal = `totalVault:boosted-uni:${from}`;
    ...
    const fromHistoricalBalance = new UserBoostedVaultBalance(
      `${event.block.number}:boosted-uni:${from}`
    );
    ...
    fromHistoricalBalance.vault = BOOSTED_UNI_VAULT_ADDRESS.toHexString();
  ...
  if (to != ZERO_ADDRESS) {
    const idToTotal = `totalVault:boosted-uni:${to}`;
    ...
    const toHistoricalBalance = new UserBoostedVaultBalance(
      `${event.block.number}:boosted-uni:${to}`
    );
    ...
    toHistoricalBalance.vault = BOOSTED_UNI_VAULT_ADDRESS.toHexString();
