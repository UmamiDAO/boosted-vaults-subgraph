...
} from "../generated/BoostedGlpWbtcVault/BoostedGlpVault";
import {
  VaultPricePerShare,
  VaultTVL,
  VaultTotalSupply,
...
export function handleDeposit(event: DepositEvent): void {
  ...
  const invRatio = BigInt.fromString("1").div(pricePerShareWBTC);
  ...
  userBalanceEvent.token = BOOSTED_WBTC_VAULT_ADDRESS.toHexString();
  ...
  userBalanceEvent.to = BOOSTED_WBTC_VAULT_ADDRESS.toHexString();
...
export function handleDeposit1(event: Deposit1Event): void {
  ...
  const userBalanceEvent = new UserBoostedBalanceEvent(
    `wbtc:deposit1:${event.transaction.hash.toHex()}:${event.transactionLogIndex.toString()}`
  );
...
export function handleWithdraw(event: WithdrawEvent): void {
  ...
  userBalanceEvent.assets = event.params.assets;
  userBalanceEvent.shares = event.params.shares;
  userBalanceEvent.from = BOOSTED_WBTC_VAULT_ADDRESS.toHexString();
...
export function handleTransfer(event: TransferEvent): void {
  ...
  if (from != ZERO_ADDRESS) {
    const idFromTotal = `totalVault:boosted-wbtc:${from}`;
    ...
    const fromHistoricalBalance = new UserBoostedVaultBalance(
      `${event.block.number}:boosted-wbtc:${from}`
    );
    ...
    fromHistoricalBalance.vault = BOOSTED_WBTC_VAULT_ADDRESS.toHexString();
  ...
  if (to != ZERO_ADDRESS) {
    const idToTotal = `totalVault:boosted-wbtc:${to}`;
    ...
    const toHistoricalBalance = new UserBoostedVaultBalance(
      `${event.block.number}:boosted-wbtc:${to}`
    );
    ...
    toHistoricalBalance.vault = BOOSTED_WBTC_VAULT_ADDRESS.toHexString();
