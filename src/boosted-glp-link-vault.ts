...
export function handleDeposit(event: DepositEvent): void {
  ...
  const invRatio = BigInt.fromString("1").div(pricePerShareLINK);
  ...
  userBalanceEvent.token = BOOSTED_LINK_VAULT_ADDRESS.toHexString();
  ...
  userBalanceEvent.to = BOOSTED_LINK_VAULT_ADDRESS.toHexString();
...
export function handleDeposit1(event: Deposit1Event): void {
  ...
  const userBalanceEvent = new UserBoostedBalanceEvent(
    `link:deposit1:${event.transaction.hash.toHex()}:${event.transactionLogIndex.toString()}`
  );
...
export function handleWithdraw(event: WithdrawEvent): void {
  ...
  userBalanceEvent.assets = event.params.assets;
  userBalanceEvent.shares = event.params.shares;
  userBalanceEvent.from = BOOSTED_LINK_VAULT_ADDRESS.toHexString();
...
export function handleTransfer(event: TransferEvent): void {
  ...
  if (from != ZERO_ADDRESS) {
    const idFromTotal = `totalVault:boosted-link:${from}`;
    ...
    const fromHistoricalBalance = new UserBoostedVaultBalance(
      `${event.block.number}:boosted-link:${from}`
    );
    ...
    fromHistoricalBalance.vault = BOOSTED_LINK_VAULT_ADDRESS.toHexString();
  ...
  if (to != ZERO_ADDRESS) {
    const idToTotal = `totalVault:boosted-link:${to}`;
    ...
    const toHistoricalBalance = new UserBoostedVaultBalance(
      `${event.block.number}:boosted-link:${to}`
    );
    ...
    toHistoricalBalance.vault = BOOSTED_LINK_VAULT_ADDRESS.toHexString();
