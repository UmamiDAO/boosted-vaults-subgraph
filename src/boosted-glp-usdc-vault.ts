...
export function handleDeposit(event: DepositEvent): void {
  ...
  const invRatio = BigInt.fromString("1").div(pricePerShareUSDC);
  ...
  userBalanceEvent.token = BOOSTED_USDC_VAULT_ADDRESS.toHexString();
  ...
  userBalanceEvent.to = BOOSTED_USDC_VAULT_ADDRESS.toHexString();
...
export function handleDeposit1(event: Deposit1Event): void {
  ...
  const userBalanceEvent = new UserBoostedBalanceEvent(
    `usdc:deposit1:${event.transaction.hash.toHex()}:${event.transactionLogIndex.toString()}`
  );
...
export function handleWithdraw(event: WithdrawEvent): void {
  ...
  userBalanceEvent.assets = event.params.assets;
  userBalanceEvent.shares = event.params.shares;
  userBalanceEvent.from = BOOSTED_USDC_VAULT_ADDRESS.toHexString();
...
export function handleTransfer(event: TransferEvent): void {
  ...
  if (from != ZERO_ADDRESS) {
    const idFromTotal = `totalVault:boosted-usdc:${from}`;
    ...
    const fromHistoricalBalance = new UserBoostedVaultBalance(
      `${event.block.number}:boosted-usdc:${from}`
    );
    ...
    fromHistoricalBalance.vault = BOOSTED_USDC_VAULT_ADDRESS.toHexString();
  ...
  if (to != ZERO_ADDRESS) {
    const idToTotal = `totalVault:boosted-usdc:${to}`;
    ...
    const toHistoricalBalance = new UserBoostedVaultBalance(
      `${event.block.number}:boosted-usdc:${to}`
    );
    ...
    toHistoricalBalance.vault = BOOSTED_USDC_VAULT_ADDRESS.toHexString();
